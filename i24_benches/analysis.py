...
def parse_args():
    ...
    parser.add_argument(
        "--input",
        "-i",
        default="i24_benchmark_results.json",
...
def preprocess_data(data):
    ...
    pivot_throughput = df.pivot(
        index="operation", columns="operand_type", values="throughput"
    )
    pivot_duration = df.pivot(
        index="operation", columns="operand_type", values="duration_ns"
    )
    ...
    if "i24" in pivot_throughput.columns and "i32" in pivot_throughput.columns:
        performance_df["throughput_ratio"] = (
            pivot_throughput["i24"] / pivot_throughput["i32"]
        )
...
def plot_throughput_comparison(data, output_dir, img_format, dpi):
    ...
    for op in throughput_df.index:
        if "i24" in throughput_df.columns and "i32" in throughput_df.columns:
            if not pd.isna(throughput_df.loc[op, "i24"]) and not pd.isna(
                throughput_df.loc[op, "i32"]
            ):
    ...
    for i, (i24_val, i32_val) in enumerate(zip(i24_throughput, i32_throughput)):
        plt.text(
            i - width / 2,
            i24_val + max(i24_throughput) * 0.02,
            f"{i24_val:,.0f}",
            ha="center",
            va="bottom",
            fontsize=8,
        ...
        plt.text(
            i + width / 2,
            i32_val + max(i32_throughput) * 0.02,
            f"{i32_val:,.0f}",
            ha="center",
            va="bottom",
            fontsize=8,
    ...
    print(
        f"Created throughput comparison chart: {output_dir}/throughput_comparison.{img_format}"
    )
    plt.close()
...
def plot_relative_performance(data, output_dir, img_format, dpi):
    ...
    sorted_df = performance_df.sort_values(
        ["category", "throughput_ratio"], ascending=[True, False]
    )
    ...
    colors = [
        category_colors[cat] if r >= 1.0 else sns.desaturate(category_colors[cat], 0.6)
    ...
    for i, (op, cat) in enumerate(zip(operations, categories)):
        if cat != last_category:
            if i > 0:
                plt.axhline(
                    y=i - 0.5, color="gray", linestyle="-", alpha=0.3, linewidth=1
                )
            last_category = cat
    ...
    print(
        f"Created relative performance chart: {output_dir}/relative_performance.{img_format}"
    )
    plt.close()
...
def plot_performance_by_category(data, output_dir, img_format, dpi):
    ...
    if (
        "throughput_ratio" not in performance_df.columns
        or "category" not in performance_df.columns
    ):
    ...
    print(
        f"Created category performance chart: {output_dir}/performance_by_category.{img_format}"
    )
    plt.close()
...
def plot_operation_durations(data, output_dir, img_format, dpi):
    ...
    for op in duration_df.index:
        if "i24" in duration_df.columns and "i32" in duration_df.columns:
            if not pd.isna(duration_df.loc[op, "i24"]) and not pd.isna(
                duration_df.loc[op, "i32"]
            ):
                operations.append(op)
    ...
    print(
        f"Created operation durations chart: {output_dir}/operation_durations.{img_format}"
    )
    plt.close()
...
def generate_markdown_report(data, output_dir):
    ...
    if not performance_df.empty:
        ...
        if "throughput_ratio" in performance_df.columns:
            performance_df["throughput_ratio"] = performance_df["throughput_ratio"].map(
                lambda x: f"{x:.2f}x" if not pd.isna(x) else np.nan
            )
        if "duration_ratio" in performance_df.columns:
            performance_df["duration_ratio"] = performance_df["duration_ratio"].map(
                lambda x: f"{x:.2f}x" if not pd.isna(x) else np.nan
            )

    # Format throughput values with commas
    for col in throughput_df.columns:
        throughput_df[col] = throughput_df[col].map(
            lambda x: f"{x:,.0f}" if not pd.isna(x) else np.nan
        )

    # Format duration values
    for col in duration_df.columns:
        duration_df[col] = duration_df[col].map(
            lambda x: f"{x:.3f}" if not pd.isna(x) else np.nan
        )
    ...
    if "throughput_ratio" in performance_df.columns:
        better_count = sum(
            performance_df["throughput_ratio"].str.rstrip("x").astype(float) >= 1.0
        )
        worse_count = sum(
            performance_df["throughput_ratio"].str.rstrip("x").astype(float) < 1.0
        )
        best_op = (
            performance_df["throughput_ratio"].str.rstrip("x").astype(float).idxmax()
        )
        best_ratio = performance_df.loc[best_op, "throughput_ratio"]
        worst_op = (
            performance_df["throughput_ratio"].str.rstrip("x").astype(float).idxmin()
        )
    ...
    if (
        "category" in performance_df.columns
        and "throughput_ratio" in performance_df.columns
    ):
        ...
        category_perf["ratio_value"] = (
            category_perf["throughput_ratio"].str.rstrip("x").astype(float)
        )
        category_avg = (
            category_perf.groupby("category")["ratio_value"]
            .mean()
        ...
        markdown.append(
            tabulate(
                category_table,
                headers=["Category", "Average Performance"],
                tablefmt="pipe",
            )
        )
        markdown.append("\n")
    ...
    markdown.append(
        tabulate(throughput_reset, headers="keys", tablefmt="pipe", showindex=False)
    )
    markdown.append("\n")
    ...
    markdown.append(
        tabulate(duration_reset, headers="keys", tablefmt="pipe", showindex=False)
    )
    markdown.append("\n")
    ...
    if not performance_df.empty:
        perf_reset = performance_df.reset_index()[
            ["operation", "throughput_ratio", "duration_ratio", "category"]
        ]
        perf_reset.columns = [
            "Operation",
            "Throughput Ratio",
        ...
        markdown.append(
            tabulate(perf_reset, headers="keys", tablefmt="pipe", showindex=False)
        )
        markdown.append("\n")
    ...
    if "throughput_ratio" in performance_df.columns:
        ...
        performance_df["ratio_value"] = (
            performance_df["throughput_ratio"].str.rstrip("x").astype(float)
        )
        ...
        markdown.append(
            tabulate(top5_table, headers="keys", tablefmt="pipe", showindex=False)
        )
        markdown.append("\n")
        ...
        bottom5_table = bottom5.reset_index()[
            ["operation", "throughput_ratio", "category"]
        ]
        bottom5_table.columns = ["Operation", "Performance Ratio", "Category"]
        markdown.append(
            tabulate(bottom5_table, headers="keys", tablefmt="pipe", showindex=False)
        )
        markdown.append("\n")
